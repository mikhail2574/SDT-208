...
def n_plus_one_bad() -> None:
    ...
    with SessionLocal() as session:
        ...
        for u in users:
            # Accessing u.posts triggers an extra query per user
...
def n_plus_one_optimized() -> None:
    ...
    with SessionLocal() as session:
        ...
        for u in users:
            # Posts are already loaded; no extra queries per user
            print(f"{u.email} has {len(u.posts)} posts")
...
def overfetching_optimized() -> None:
    """Fetch only Post.id and Post.title as a projection."""
...
def pagination_optimized(page: int = 1, page_size: int = 10) -> None:
    ...
    with SessionLocal() as session:
        ...
        posts = session.scalars(stmt).all()
        print(
            f"Page {page} (page size {page_size}) -> {len(posts)} posts loaded."
        )
        for p in posts:
